...
export const useAccommodationStore = defineStore('accommodation', () => {
  const roomTypes = ref([
    { type: 'no_accommodation', name: 'Без размещения в номерах', capacity: 0, available: Infinity },
  ...
  const accommodationOptions = computed(() => {
    ...
    return findAccommodationOptions(
      peopleCount.value,
      roomTypes.value.filter(room => 
        enabledRoomTypes.value.includes(room.type) && room.type !== 'no_accommodation'
      )
    )
  })
